...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < list_size / 3; i++)
    {
        if (0 == myrank)
        {
            ...
            for (int p = 0; p < 3; p++)
            {
                helper_array[p * 2] = list[i * 3 + p + cycle1];
                helper_array[p * 2 + 1] = list[i * 3 + p + cycle2];
            }
        ...
        if (0 == myrank)
            MPI_Gatherv(subresults.data(), amounts_rec[myrank], MPI_INT, list.data() + i * 3, amounts_rec.data(), disps_rec.data(), MPI_INT, 0, MPI_COMM_WORLD);
        else
